...
await app.register(swagger, { openapi: { openapi: '3.0.3', info: { title: 'ssh-bridge REST API', version: '0.1.0' }, tags: [{ name: 'server' }, { name: 'exec' }, { name: 'terminal' }, { name: 'file' }] } });
...
app.get('/api/files/stat', { schema: { tags: ['file'], summary: '获取远程文件信息', querystring: { type: 'object', required: ['server','path'], properties: { server: { type: 'string' }, path: { type: 'string' } } } } }, async (r) => { const q = r.query as { server: string; path: string }; return bridge.statFile(q.server, q.path); });
app.get('/api/files', { schema: { tags: ['file'], summary: '列出远程目录', querystring: { type: 'object', required: ['server','path'], properties: { server: { type: 'string' }, path: { type: 'string' } } } } }, async (r) => { const q = r.query as { server: string; path: string }; return bridge.listFiles(q.server, q.path); });
app.get('/api/files/content', { schema: { tags: ['file'], summary: '读取远程文件', querystring: { type: 'object', required: ['server','path'], properties: { server: { type: 'string' }, path: { type: 'string' }, offset: { type: 'integer', minimum: 0, default: 0 }, maxBytes: { type: 'integer', minimum: 1, maximum: 1048576, default: 1048576 } } } } }, async (r) => { const q = r.query as { server: string; path: string; offset?: number; maxBytes?: number }; return bridge.readFile(q.server, q.path, q.offset, q.maxBytes); });
